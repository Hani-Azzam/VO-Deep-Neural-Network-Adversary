...
class MultiTrajFolderDatasetCustom(Dataset):
    ...
    def __init__(self, rootfolder, processed_data_folder="", preprocessed_data=False,
                 ...
                 perspective_padding=(0, 0), dataset_indices=None):
        ...
        if dataset_indices is None:
            ...
            self.datasets_names = [ff for ff in rootfolder_files
                                   if os.path.isdir(rootfolder + '/' + ff) and int(ff) in dataset_indices]
        ...
        self.traj_len = None
        tot_files_num = 0
        for dataset_idx, dataset_name in zip(dataset_indices, self.datasets_names):
...
class MultiTrajFolderDatasetRealData(Dataset):
    ...
    def __init__(self, rootfolder, processed_data_folder="", preprocessed_data=False,
        ...
        if dataset_indices is None:
            ...
            traj_folders = [ff for ff in rootfolder_files
                            if os.path.isdir(rootfolder + '/' + ff) and int(ff) in dataset_indices]
        traj_folders.sort()
