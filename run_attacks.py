...
def clean_multi_inputs(args, dataset: str):
    dataset_idx_list = []
    dataset_name_list = []
    traj_name_list = []
    traj_indices = []
    ...
    print(f"len(args.{dataset}Dataloader)")
    print(len(getattr(args, f"{dataset}Dataloader")))
    for traj_idx, traj_data in enumerate(getattr(args, f"{dataset}Dataloader")):
...
def report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                         traj_clean_crit_list, traj_adv_crit_list, save_csv,
                         output_dir, crit_str,
                         experiment_name=""):
...
def run_attacks_train(args):
    ...
    print("A single universal will be produced and then tested on the dataset")
    print("args.attack")
    print(args.attack)
    attack = args.attack_obj

    dataset_idx_list, dataset_name_list, traj_name_list, traj_indices, \
    test_motions_gt_list, traj_clean_criterions_list, traj_clean_motions = \
        clean_multi_inputs(args, "test")
    train_motions_gt_list = clean_multi_inputs(args, "train")[4]
    val_motions_gt_list = clean_multi_inputs(args, "validation")[4]

    print("traj_name_list")
    print(traj_name_list)

    motions_target_list = train_motions_gt_list
    ...
    best_pert, clean_loss_list, all_loss_list, all_best_loss_list, total_loss_track = \
        attack.perturb(args.trainDataloader, motions_target_list, eval_data_loader=args.validationDataloader,
                       eval_y_list=val_motions_gt_list,
                       eps=args.eps, device=args.device)
    ...
    traj_adv_criterions_list = \
        test_adv_trajectories(args.testDataloader, args.model, test_motions_gt_list, attack, best_pert,
    ...
    if args.save_csv:
        pd.DataFrame(total_loss_track.numpy()).to_csv(os.path.join(args.output_dir, f"results_total_loss_track.csv"), )
