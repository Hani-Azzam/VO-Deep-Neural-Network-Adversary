...
class VOCriterion:
    def __init__(self, t_crit='rms', rot_crit='none', flow_crit='none', target_t_crit='none',
        ...
        if t_crit in self.criterions:
            self.calc_t_crit = self.criterions[t_crit]
        else:
    ...
    # TODO: Define criterions under this class as methods and add to critertions dict.
    _criterions = None

    @property
    def criterions(self):
        if self._criterions is None:
            self._criterions = {
                "partial_rms": self.calc_partial_poses_t,
                "mean_partial_rms": self.calc_mean_partial_poses_t,
            }
