...
from attacks import PGD, APGD, Const
...
def parse_args():
    ...
    parser.add_argument('--save_report_run', action="store_true",
                        default=False, help='report to results directory (default: False)')
    parser.add_argument('--experiment_name', type=str, default=None)

    # data loader params
    parser.add_argument('--test_set_idx', type=int, default=4)
    parser.add_argument('--val_set_idx', type=int, default=3)
    ...
    parser.add_argument("--attack_optimizer", type=str, metavar="ATTOPT", default="I_FGSM",
                        help="attack optimizer")
    # apgd
    parser.add_argument('--sparsity_ratio_threshold', type=float, default=0.9)
    parser.add_argument('--checkpoint_step_size', type=int, default=5)
...
def compute_data_args(args):
    ...
    def create_dataset_dataloader(dataset_indices):
        dataset = args.dataset_class(
            args.test_dir, processed_data_folder=args.processed_data_dir,
        ...
        return dataset, DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.worker_num)

    dataset_idx_split = DatasetSplit(
        train=tuple(idx for idx in range(5) if idx not in [args.test_set_idx, args.val_set_idx]),
        test=(args.test_set_idx, ),
        validation=(args.val_set_idx, ),
    )
    args.trainDataset, args.trainDataloader = create_dataset_dataloader(dataset_idx_split.train)
    args.validationDataset, args.validationDataloader = create_dataset_dataloader(dataset_idx_split.validation)
    args.testDataset, args.testDataloader = create_dataset_dataloader(dataset_idx_split.test)
...
def compute_attack_args(args):
    ...
    attack_dict = {'pgd': PGD, "apgd": APGD, 'const': Const}
    ...
    if args.attack not in attack_dict:
        ...
        if args.attack_name == 'const':
            ...
            attack_obj_kwargs = {
                "optimizer": args.attack_optimizer,
                "beta_1": args.beta_1,
                "beta_2": args.beta_2,
                "mu": args.mu,
            }
            if args.attack is APGD:
                assert args.attack_optimizer in {"MI_FGSM", "I_FGSM"}
                attack_obj_kwargs["sparsity_ratio_threshold"] = args.sparsity_ratio_threshold
                attack_obj_kwargs["checkpoints"] = range(args.checkpoint_step_size - 1, args.attack_k,
                                                         args.checkpoint_step_size)
...
def compute_output_dir(args):
    ...
    if args.attack is None:
        ...
        if args.attack_name == 'const':
            ...
            attack_opt_string = args.attack_optimizer
            ...
            args.output_dir += f"/seed_{args.seed}_"
            args.output_dir += "eps_" + str(args.eps).replace('.', '_') + \
                               "_attack_iter_" + str(args.attack_k) + \
                               "_alpha_" + str(args.alpha).replace('.', '_')
            if args.attack_optimizer in ["AB_FGSM", "M_AB_FGSM", "AI_FGSM"]:
                args.output_dir += f"_beta_1_{str(args.beta_1).replace('.', '_')}"
                args.output_dir += f"_beta_2_{str(args.beta_2).replace('.', '_')}"
            elif args.attack_optimizer in ["MI_FGSM"]:
                args.output_dir += f"_mu_{str(args.mu).replace('.', '_')}"
            if args.attack_name == "apgd":
                args.output_dir += f"_sparsity_ratio_threshold_{str(args.sparsity_ratio_threshold).replace('.','_')}"

            if not isdir(args.output_dir):
                mkdir(args.output_dir)

    if args.experiment_name is not None:
        args.output_dir += f"/{args.experiment_name}"
        if not isdir(args.output_dir):
            mkdir(args.output_dir)

    args.output_dir += f"/test_set_idx_{args.test_set_idx}_val_set_idx_{args.val_set_idx}"
    if not isdir(args.output_dir):
        mkdir(args.output_dir)
...
def get_args():
    ...
    if args.save_report_run:
        sys.stdout = open(args.output_dir + "/run_report", "w")
